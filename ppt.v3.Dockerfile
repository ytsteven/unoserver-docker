...
# FROM alpine:3.18.3
FROM hongyaa-docker.pkg.coding.net/qingjiao/service-pub/ubuntu:24.04
...
# Install unoserver
RUN pip3 install -U --break-system-packages unoserver

# Set up Supervisor
COPY --chown=${UID}:${GID} ${BUILD_CONTEXT} /opt/qingjiao-service
RUN chmod +x /opt/qingjiao-service/entrypoint.sh && \
...
VOLUME ["/data"]
EXPOSE 2003
ENTRYPOINT ["/opt/qingjiao-service/entrypoint.sh"]
